...
function fetchAndDisplayGif(event) {
    ...
    var validation = $("#validation input[name=tag2]").val();
    if (validation != "5") {
        $("#feedback").text("No GIFs for you!");
//        $("#validation style=[bordercolor=red]").val();
...
//            console.log("we received a response cool!");
//            console.log(response);
            ...
            for(var property in content) {
                if(property = "image_url") {
                    imgHTML = response.data[property];}
...
//            console.log("this is it " + imgHTML);
            $("#gif").attr({ src: imgHTML, hidden: null});
            setGifLoadedStatus(true);
        },
        error: function() {
            ...
            $("#feedback").text("Sorry, could not load GIF buddy. Try again!");
            setGifLoadedStatus(false);
        },
